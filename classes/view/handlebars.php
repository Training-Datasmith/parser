...
class View_Handlebars extends \View
{
    ...
    protected function process_file($file_override = false)
    {
        ...
        if (!is_file($compiled) or filemtime($file) > filemtime($compiled) or \Config::get('parser.View_Handlebars.force_compile', true)) {
            ...
            $ext = $this->extension;
            file_put_contents($compiled, '<?php ' . Lightn_Candy::compile(file_get_contents($file), ['partialresolver' => function ($cx, $name) use ($ext): string|false {
                $partial = \Finder::search('views', $name, '.' . $ext, false, false);
                return empty($partial) ? "[ PARTIAL {$name} NOT FOUND!]" : file_get_contents($partial);
            }] + \Config::get('parser.View_Handlebars.environment', [])), LOCK_EX);
        }
        ...
        try {
            ...
            if (!is_callable($renderer)) {
                throw new \Fuel_Exception('Compiled Handlebars template did not return a callable: ' . $compiled);
            }
